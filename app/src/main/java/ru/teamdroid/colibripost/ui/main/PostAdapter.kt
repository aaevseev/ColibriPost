    ...
    class PostViewHolder(val root: View) : BaseAdapter.BaseViewHolder(root) {
        ...
        override fun onBind(item: Any) {
            (item as? PostEntity)?.let {
                ...
                PicassoHelper.loadImageFile(
                    root.context,
                    item.channelPhotoPath,
                    root.ivChannelPhoto
                )
                ...
                calendar.time = date
                root.tvPostDate.text = calendar.get(Calendar.HOUR_OF_DAY)
                    .toString() + ":" + calendar.get(Calendar.MINUTE).toString()
            }
        }

    }

    class HeaderPostViewHolder(val root: View) :
       BaseAdapter.BaseViewHolder(root) {
